...
f = open(r'C:\Users\geete\Downloads\usercode\bank_app\build\contracts\Bank.json')
data = json.load(f)
abi = data['abi']
...
def get_history():
    ...
    for num in blocks:
        ...
        if str(cd[0]) == '<Function transfer_funds(address,uint256)>':
            transactions.append({'id': str(tx.blockNumber), 'amount': str(cd[1]['_funds']), 'sentTo': str(cd[1]['receiving_address'])})
